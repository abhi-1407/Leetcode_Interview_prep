...
public:
	...
	// seg[ind]=seg[2*ind+1]+ seg[2*ind+2]; -> This will help populate all other intermediate nodes
	...
	// ind       -> represents "position" in the segment tree data structure (Ex: 0 -> root)
	// Using left, right and ind -> we have all the information on the segment tree
	// Node at 'ind' in segment tree will have children at 2ind+1(left) and 2ind+2(right)
	...
	// Otherwise, we need to find the index and we do this by checking child nodes (2ind+1, 2ind+2)
	//      update the segment tree ind with the updated child values' sum.
	...
	// ind       -> represents "position" in the segment tree data structure (Ex: 0 -> root)
	// Using low, high and ind -> we have all the information on the segment tree
	// Node at 'ind' in segment tree will have children at 2ind+1(left) and 2ind+2(right)
	...
	int rangeUtil(int qlow, int qhigh, int low, int high, int ind) {
		...
		return (rangeUtil(qlow, qhigh, low, mid, 2 * ind + 1) + rangeUtil(qlow, qhigh, mid + 1, high, 2 * ind + 2));
	}
	...
	int sumRange(int left, int right) {
		...
		// query from left to right while segment tree is now at 'root' (ind=0) and range(0, n-1)
	}
};
